...
const Sidebar = () => {
  ...
  return (
    <>
      <div className={`w-20 ${bgClass} border-r flex flex-col items-center py-6 gap-6 z-20 shadow-2xl`}>
        {/* Logo */}
        ...
        <div className="flex-1 flex flex-col gap-4 w-full items-center">
          {/* Search */}
          <button onClick={() => { closeAll(); setIsSearchOpen(true); }} className={`p-3 ${textClass} rounded-xl transition-all relative group`}>
            <Search size={22} />
          ...
          <div className="relative">
            <button onClick={() => { setIsNotifOpen(!isNotifOpen); setIsSettingsOpen(false); setIsProfileOpen(false); }} className={`p-3 ${textClass} rounded-xl transition-all relative`}>
              <Bell size={22} />
            ...
            <AnimatePresence>
              {isNotifOpen && (
                <motion.div
                  ...
                  className={`absolute left-16 top-0 w-72 ${dropdownClass} border rounded-2xl shadow-2xl p-3 z-50`}
                >
          ...
          {/* Settings */}
          <div className="relative">
            <button onClick={() => { setIsSettingsOpen(!isSettingsOpen); setIsNotifOpen(false); setIsProfileOpen(false); }} className={`p-3 ${textClass} rounded-xl transition-all`}>
              <Settings size={22} />
            </button>
            ...
            <AnimatePresence>
              {isSettingsOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: -10, scale: 0.95 }}
                  transition={{ duration: 0.15 }}
                  className={`absolute left-16 top-0 w-64 ${dropdownClass} border rounded-2xl shadow-2xl overflow-hidden z-50`}
                >
